      ...
      <div className="relative z-10 flex flex-col items-center p-4 text-center max-w-4xl w-full mx-auto">
        ...
        >
          {title.split('').map((char, index) => (
            <motion.span key={index} variants={letterVariants} className="inline-block" style={{ textShadow: `0 2px 10px rgba(0,0,0,0.5)` }}>
              {char === ' ' ? '\u00A0' : char}
            </motion.span>
          ))}
        </motion.h1>

        <motion.p variants={itemVariants} className="mt-4 text-lg md:text-xl text-brand-nav-muted max-w-2xl">
        ...
        <motion.div variants={itemVariants} className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4 w-full">
          <Link to="/admissions" className="w-full sm:w-auto">
            <AnimatedButton className="w-full sm:w-auto">Begin Your Application</AnimatedButton>
          </Link>
