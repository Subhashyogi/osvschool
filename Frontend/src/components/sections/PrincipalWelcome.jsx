...
const PrincipalWelcome = () => {
    ...
    return (
        <section ref={sectionRef} className="py-16 md:py-20 bg-gray-50 relative overflow-hidden">
            <span aria-hidden="true" className="absolute top-0 right-0 text-[20rem] font-bold text-gray-200/50 -z-0 pointer-events-none" style={{ transform: 'translate(20%, -20%)' }}>
                “
            </span>
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">
                    <div className="lg:col-span-2 flex justify-center">
                        <motion.div style={{ y: parallaxY }} className="relative w-64 h-64 md:w-80 md:h-80">
                            <div className="absolute inset-0 bg-brand-accent rounded-full transform rotate-6"></div>
                            <img src={principal.image} alt={principal.name} className="relative w-full h-full rounded-full object-cover border-8 border-brand-white shadow-2xl" />
                        </motion.div>
                    </div>
                    <motion.div
