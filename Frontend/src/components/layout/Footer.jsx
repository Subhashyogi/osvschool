...
import { FaFacebookF, FaTwitter, FaInstagram, FaArrowRight, FaUniversity, FaBookOpen, FaUserFriends, FaInfoCircle } from 'react-icons/fa';

const sitemapData = [
    {
        title: "About Us", icon: <FaInfoCircle className="text-brand-accent" />,
        links: [{ label: "Our Story", to: "/about" }, { label: "Faculty", to: "/faculty" }, { label: "Contact", to: "/contact" }]
    },
    {
        title: "Academics", icon: <FaBookOpen className="text-brand-accent" />,
        links: [{ label: "Programs", to: "/academics" }, { label: "Campus Life", to: "/gallery" }]
    },
    {
        title: "Admissions", icon: <FaUniversity className="text-brand-accent" />,
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
                                ...
                                <div key={section.title}>
                                    ...
                                    <ul className="space-y-3 text-sm">
                                        {section.links.map(link => (
                                            <li key={link.label}><Link to={link.to} className="hover:text-brand-accent transition-colors">{link.label}</Link></li>
                                        ))}
                                    </ul>
                                </div>
                            ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    ...
                    <form className="flex flex-col sm:flex-row gap-2 w-full max-w-md md:ml-auto">
                        <input type="email" placeholder="your@email.com" className="w-full text-sm px-4 py-3 bg-brand-surface/80 text-brand-light rounded-md focus:ring-2 focus:ring-brand-accent focus:outline-none placeholder:text-brand-nav-muted" aria-label="Email for newsletter" />
                        <button type="submit" className="flex-shrink-0 px-4 py-3 bg-brand-accent text-brand-dark rounded-md hover:bg-opacity-90" aria-label="Subscribe">
            ...
            <div className="border-t border-brand-surface/50">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row justify-between items-center text-sm text-brand-nav-muted/70">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <Link to="#" className="hover:text-brand-accent">Terms of Use</Link>
                    </div>
                </div>
            </div>
        </footer>
