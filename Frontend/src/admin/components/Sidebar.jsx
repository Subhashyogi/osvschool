...
import { FaTachometerAlt, FaUsers, FaImages, FaSignOutAlt, FaSchool, FaTrophy, FaCommentAlt } from 'react-icons/fa';
import { useAuth } from '../context/AuthContext';

const Sidebar = ({ isSidebarOpen, setSidebarOpen }) => {
    const { logout } = useAuth();

    const baseLinkClass = "flex items-center px-4 py-3 text-gray-200 transition-colors duration-200 transform rounded-md";
    ...
    return (
        <>
            ...
            <div
                className={`fixed inset-y-0 left-0 flex flex-col w-64 bg-brand-nav text-gray-200 transform z-30 transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
            >
