...
import { facultyMembers, facultyStats, featuredFaculty, departments } from '../constants/index';
        ...
        <div className="bg-brand-light text-brand-dark">
            <div className="relative pt-32 md:pt-40 pb-20 md:pb-24 text-center bg-cover bg-center" style={{ backgroundImage: "url('https://images.pexels.com/photos/159711/books-bookstore-book-reading-159711.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2')" }}>
                ...
                <div className="relative container mx-auto px-4">
                    ...
                    <p className="mt-4 text-base md:text-lg max-w-2xl mx-auto text-brand-nav-muted">The heart of our institution—a dedicated team of mentors, innovators, and scholars.</p>
                </div>
            </div>
            ...
            <section className="py-16 md:py-24">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-brand-dark">Faculty Spotlight</h2>
                    <motion.div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-8 md:gap-12 bg-white p-8 md:p-12 rounded-xl shadow-xl" initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.6 }}>
                        ...
                        <div className="text-center md:text-left">
                            <h3 className="text-2xl md:text-3xl font-bold text-brand-dark">{featuredFaculty.name}</h3>
                            <p className="text-lg md:text-xl text-brand-accent font-semibold mt-1">{featuredFaculty.title}</p>
                            <p className="mt-4 text-brand-muted leading-relaxed italic">"{featuredFaculty.quote}"</p>
                        </div>
                    </motion.div>
                </div>
            </section>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex justify-center flex-wrap gap-2 md:gap-4 mb-12">
                        {departments.map(department => (
                            <button key={department.name} onClick={() => setActiveFilter(department.name)} className={`flex items-center gap-2 px-4 py-2 text-sm md:text-md font-semibold rounded-full transition-colors duration-300 relative focus:outline-none ${activeFilter === department.name ? 'text-brand-light bg-brand-surface' : 'text-brand-nav-muted hover:text-brand-light'}`}>
                                {department.icon} <span>{department.name}</span>
                                {activeFilter === department.name && (<motion.div className="absolute bottom-[-4px] left-2 right-2 h-0.5 bg-brand-accent" layoutId="faculty-underline" />)}
                            </button>
                        ))}
                    </div>
