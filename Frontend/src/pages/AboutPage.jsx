        ...
        <div className="bg-brand-light text-brand-dark">
            <header ref={headerRef} className="relative h-[60vh] overflow-hidden">
                <motion.div
                    ...
                    style={{ y: parallaxY, backgroundImage: `url('https://images.pexels.com/photos/8617830/pexels-photo-8617830.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2')` }}
                />
                ...
                <div className="relative z-20 h-full flex flex-col justify-center items-center text-center px-4">
                    ...
                    <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="mt-4 text-lg md:text-xl max-w-2xl text-brand-nav-muted">A tradition of nurturing talent and fostering innovation since 1985.</motion.p>
                </div>
            </header>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="relative max-w-2xl mx-auto">
                        ...
                        {timelineEvents.map((event, index) => (
                            <div key={index} className={`mb-16 flex w-full items-center ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
                                ...
                                <motion.div initial={{ opacity: 0, x: index % 2 === 0 ? -50 : 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.6 }} className={`w-[calc(50%-2rem)] p-6 bg-white shadow-lg rounded-2xl ${index % 2 === 0 ? 'mr-auto' : 'ml-auto'}`}>
                                    <p className="text-brand-accent font-bold text-lg">{event.year}</p>
                                    <h3 className="text-xl font-bold mt-1 text-brand-dark">{event.title}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {[
                            ...
                            <motion.div key={index} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} viewport={{ once: true }} className="relative h-80 rounded-2xl overflow-hidden shadow-xl group">
                                <img src={item.image} alt={item.value} className="absolute inset-0 z-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                <div className="absolute inset-0 bg-gradient-to-t from-brand-dark/80 to-transparent z-10" />
                                <div className="relative z-20 flex flex-col justify-end h-full p-6 text-brand-light">
            ...
            <section className="bg-gray-50 py-16 text-center">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 className="text-2xl md:text-3xl font-bold text-brand-dark">Join Our Legacy</h2>
